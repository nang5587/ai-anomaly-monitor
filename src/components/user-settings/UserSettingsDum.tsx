...
import { getMyProfile, updateProfileInfo, changePassword, deleteMyAccount } from '@/api/userApi';
...
export default function UserSettings({ initialProfile }: UserSettingsProps) {
    ...
    const handlePasswordSave = async (e: React.FormEvent) => {
        ...
        try {
            ...
            // ❗️ API 응답에 따라 성공 여부 분기 처리가 필요할 수 있습니다.
            //    예: const result = await changePassword(...); if (result.success) { ... }
            toast.success("비밀번호가 성공적으로 변경되었습니다.");
            setPasswordData({ password: '', newPassword: '', confirmPassword: '' });
        } catch (error: any) {
            // ✨ API 클라이언트가 에러 응답 본문을 반환하도록 설정했다면,
            //    더 구체적인 에러 메시지를 보여줄 수 있습니다.
    ...
    const handleDeleteAccount = async () => {
        ...
        try {
            // ✨ mockDeleteUser -> deleteMyAccount (userApi.ts에 정의한 함수)
            await deleteMyAccount();

            toast.success("회원 탈퇴가 완료되었습니다. 로그인 페이지로 이동합니다.");

            // ✨ AuthContext의 실제 logout 함수를 호출하여 토큰 등을 정리
            logout();
                    ...
                    <div>
                        ...
                        <form id="profileForm" onSubmit={handleProfileSave} className="space-y-4">
                            <div>
                                ...
                                <input value={user?.userId} readOnly className="w-full bg-[rgba(20,20,20)] border-none rounded-md p-2 text-gray-500 cursor-not-allowed" />
                            </div>
                            <div>
